...
class MonsterSiegeApp:
    ...
    def build(self):
        ...
        self.base_bar.grid(row=2, column=0, sticky="ew", pady=(0, 3))
        ...
        self.encounter_info = ttk.Label(encounter, text="Click a monster on the map.", justify="center")
        ...
        ttk.Button(controls, text="Advance 10 sec", command=self.advance).pack(side="left", padx=(0, 6))
        self.boss_button = ttk.Button(controls, text="⚔ Attack Boss", command=self.boss_attack, state="disabled")
        self.boss_button.pack(side="left", padx=(0, 6))
        ttk.Button(controls, text="Reset", command=self.reset).pack(side="right")

        self.status = ttk.Label(root, font=("Segoe UI", 10, "bold"))
        self.status.grid(row=6, column=0, sticky="ew", pady=(4, 0))
